                            ...
                            <div class="border rounded-0 shadow bg-gradient-navy px-1 py-1" id="sale-panel">
                                <div class="d-flex h-100 w-100">
                                                                ...
                                                                <?php if (isset($prod_arr[$k])) : ?>
                                                                    <?php foreach ($prod_arr[$k] as $row) : ?>
                                                                        <div class=" col-lg-3 col-md-4 col-sm-12 col-xs-12 px-2 py-3">
                                                                            <div class="card rounded-pill text-dark text-decoration-none " data-price="<?= $row['unit_price'] ?>" data-id="<?= $row['item_id'] ?>">
                                                                                <div class="card-body text-center prod-item">
                                                                                    <?= $row['item_name'] ?>
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                    ...
                                    <div class="col-5 h-100">
                                        <table id="product-list" class="table table-bordered table-striped mb-0">
                                            <thead>
                                                <tr class="bg-gradient-navy-dark">
                                                    ...
                                                    <th class="text-center px-2 py-1">Xóa</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr></tr>
...
<script>
  function calc_change() {
    ...
    $('.prod-item').click(function() {
        alert('d');
    // var id = $(this).attr('data-id'); // Lấy ID sản phẩm
    // var name = $(this).text().trim(); // Lấy tên sản phẩm
    // var price = $(this).attr('data-price'); // Lấy giá sản phẩm

    // // Tạo một dòng mới trong bảng
    // var newRow = $('<tr>');
    
    // // Thêm ô số lượng
    // newRow.append('<td><input type="number" name="product_qty[]" class="form-control" value="1" min="1"></td>');

    // // Thêm ô tên món
    // newRow.append('<td class="product_name">' + name + '</td>');

    // // Thêm ô giá
    // newRow.append('<td><input type="text" name="product_price[]" class="form-control" value="' + price + '" readonly></td>');

    // // Thêm nút xóa sản phẩm
    // newRow.append('<td><button type="button" class="btn btn-danger rem-product">Xóa</button></td>');
